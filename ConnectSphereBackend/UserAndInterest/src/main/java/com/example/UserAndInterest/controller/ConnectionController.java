...
public class ConnectionController {
    ...
    public ResponseEntity<ApiResponse> connectUser(
        ...
        ApiResponse response =
                connectionService.connectUser(requesterUserId, recipientUserId);
    ...
    public ResponseEntity<ApiResponse> disconnectUsers(
        ...
        ApiResponse response =
                connectionService.disconnectUsers(
                        firstUserId,
                        secondUserId
                );
    ...
    @PostMapping("/connect/{userId}/{targetUserId}")
    public ResponseEntity<ApiResponse> connect(@PathVariable String userId, @PathVariable String targetUserId) {
        return respond(connectionService.connect(userId, targetUserId));
    }

    @DeleteMapping("/disconnect/{userId}/{targetUserId}")
    public ResponseEntity<ApiResponse> disconnectSelf(@PathVariable String userId, @PathVariable String targetUserId) {
        return respond(connectionService.disconnect(userId, targetUserId));
    }

    @GetMapping("/mine/{userId}")
    public ResponseEntity<ApiResponse> mine(@PathVariable String userId) {
        return respond(connectionService.listMyConnections(userId));
    }

    @GetMapping("/search/{userId}")
    public ResponseEntity<ApiResponse> search(@PathVariable String userId,
                                              @RequestParam(required = false) String query,
                                              @RequestParam(required = false, defaultValue = "20") int limit) {
        return respond(connectionService.searchMyConnections(userId, query, limit));
    }
    ...
    public ResponseEntity<ApiResponse> status(@PathVariable String userId, @PathVariable String targetUserId) {
        boolean connected = connectionService.areConnected(userId, targetUserId);
        return ResponseEntity.ok(ApiResponse.success("Status fetched", Map.of("connected", connected)));
    }

    @GetMapping("/discover/{userId}")
    public ResponseEntity<ApiResponse> discover(@PathVariable String userId) {
        return respond(connectionService.discoverUsers(userId));
    }
    private ResponseEntity<ApiResponse> respond(ApiResponse response) {
        return response.isSuccess()
