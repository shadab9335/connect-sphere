 ...
 * deliberately no senderId in the published payload.
 */
export default function useChatSocket({ token, userId, conversationId, onMessage, onNotification, onReadReceipt }) {
    const [connected, setConnected] = useState(false);
    ...
    // 4. Read receipts — fires when the other person reads a conversation,
    //    so blue ticks update instantly without waiting for polling.
    useEffect(() => {
        const client = clientRef.current;
        if (!connected || !client || !userId) return undefined;

        const subscription = client.subscribe(`/topic/user/${userId}/read`, (frame) => {
            try {
                onReadReceiptRef.current?.(JSON.parse(frame.body));
            } catch (error) {
                console.error("[chat socket] bad read-receipt frame", error);
            }
        });
