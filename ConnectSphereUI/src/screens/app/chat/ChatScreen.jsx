...
function ChatScreen({ profilePic, chatTarget, onChatTargetHandled }) {
  ...
  const handleReadReceipt = useCallback((event) => {
    // event = { conversationId, readByUserId, lastReadMessageId }
    setActive((prev) =>
      prev && prev.id === event.conversationId
        ? { ...prev, otherLastReadMessageId: event.lastReadMessageId }
        : prev
    );
    setConversations((prev) =>
      prev.map((c) =>
        c.id === event.conversationId
          ? { ...c, otherLastReadMessageId: event.lastReadMessageId }
          : c
      )
    );
  }, []);
    ...
    return (
      <div style={SHELL}>
        ...
        <MessageList
          ...
          initialUnreadCount={active.initialUnreadCount || 0}
        />
