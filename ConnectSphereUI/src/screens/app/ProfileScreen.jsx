    ...
    if (showMyPosts) {
        return (
            <MyPostsScreen
                ...
                // "Saved" now lives as a link inside the Posts tab rather than its own stat tile.
                onViewSaved={() => { setShowMyPosts(false); setShowSavedPosts(true); }}
            />
        );
    }
                        ...
                        {[
                            ...
                            { count: String(myHostedEvents.length), label: "Events", onClick: () => setShowMyEvents(true) },
                            {
                                count: String(myPosts.length), label: "Posts", onClick: () => {
                                    // Refresh the saved-posts count so the "🔖 Saved Posts" badge
                                    // inside the Posts tab isn't stale (see MyPostsScreen fix).
                                    const stored = JSON.parse(localStorage.getItem('user') || '{}');
                                    if (stored.id) {
                                        fetchUserBookmarks(stored.id)
                                            .then(r => setSavedPosts(r.data.data || []))
                                            .catch(err => console.error('Failed to refresh saved posts count', err));
                                    }
                                    setShowMyPosts(true);
                                }
                            },
